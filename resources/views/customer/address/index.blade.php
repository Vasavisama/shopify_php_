    ...
    @if($addresses->isEmpty())
        <div class="card">
            <div class="card-body">
                ...
                <a href="{{ route('customer.address.create') }}" class="btn btn-primary">Add a new address</a>
            </div>
        </div>
    @else
        <div class="row">
                ...
                <div class="col-md-6 mb-4">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form action="{{ route('customer.address.select') }}" method="POST" class="d-inline">
                                @csrf
                                <input type="hidden" name="address_id" value="{{ $address->id }}">
                                <button type="submit" class="btn btn-primary">Select Address</button>
                            </form>
                            <a href="{{ route('customer.address.edit', $address->id) }}" class="btn btn-secondary">Edit</a>
                        </div>
                    </div>
                </div>
            @endforeach
